...
def read_datafile_tokens_memmap(path: Union[str, Path], dtype: Optional[np.dtype] = None) -> np.memmap:
    ...
    path = os.fspath(path)
    header_bytes = 256 * 4
    nbytes = read_datafile_token_dtype(path)
    if dtype is None:
        if nbytes == 1:
            dtype = np.uint8
        elif nbytes == 2:
...
class PrecachedDatasetMixin:
                ...
                for tokens in imap_iter:
                    if token_count + len(tokens) < self._shard_size:
                        ...
                        if progress_bar is None:
                            progress_bar = tqdm(total=self._shard_size, unit="tokens", desc=f"Shard {shard_index}")
    ...
    def setup_cache_index(self, split: Union[str, "Enum"], seq_len: int) -> None:
        ...
        self._memmaps = [read_datafile_tokens_memmap(p) for p in self._files]
        self._shard_sizes = np.array([read_datafile_token_count(p) for p in self._files], dtype=np.int64)
