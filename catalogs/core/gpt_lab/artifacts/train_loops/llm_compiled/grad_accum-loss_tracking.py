    ...
    **kwargs,
    ...
    for batch in train_loader:
        ...
        if micro_idx % accum_steps == 0:
            ...
            # Track loss if enabled - track on actual optimizer steps
            if track_loss and (step_count % log_interval == 0):
                # For gradient accumulation, we need to track the original loss value
    ...
    if micro_idx % accum_steps != 0:
        ...
        # Track loss for the final incomplete accumulation step if enabled
        if track_loss and (step_count % log_interval == 0):
            # For the final step, calculate the average loss from the incomplete window
            original_loss = loss * float(accum_steps) if accum_steps > 1 else loss
            train_loss_history.append(float(original_loss.detach().cpu().item()))
