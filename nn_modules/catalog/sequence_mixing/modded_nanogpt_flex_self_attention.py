...
from nn_modules.catalog.channel_mixing import FP8Linear, is_hopper_available
from nn_modules.catalog.norms import RMSNorm
from nn_modules.catalog.sequence_mixing.flex_self_attention import (
    HalfTruncatedRotary, fsa_dims_to_test, fsa_num_heads_to_test, fsa_seq_lens_to_test, fsa_dtypes_to_test
)
...
def benchmark_input_provider(init_args: dict, device: str) -> tuple:
    return (
        ...
        torch.randn(init_args['max_seq_len'], init_args['dim'], device=device, dtype=init_args['dtype'], requires_grad=True),
        block_mask
    )
